...
d3.json("https://<?php include '../../base.php'; echo $configWhoami; ?>/api/jobs/<?php echo $_GET['job']; ?>.json", function(error, graph) {
	...
	var circle = node.append("path")
		...
		.style(tocolor, function(d) {
			if (isNumber(d.score) && d.score >= -1) return color(d.score);
...
function vis_by_node_score(score) {
	if (isNumber(score)) {
		if (score >= 0.333) return keyh;
		else if (score >= -0.333) return keym;
		else if (score >= -1) return keyl;
	}
	return true;
}
...
function vis_by_link_score(score) {
	if (isNumber(score)) {
		if (score >= 0.333) return key3;
		else if (score >= -0.333) return key2;
		else if (score >= -1) return key1;
	}
	return true;
}
